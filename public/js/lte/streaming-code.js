            ...
            if (key == 'CHANGE24HOURPCT') {
                if(current[key] != 'NaN%') {
                    if(current[key].slice(0, -1) < 0) {
                        $('#' + key + '_' + from).addClass('bg-red');
                        $('#' + key + '_' + from).removeClass('bg-green');
                    } else {
                        $('#' + key + '_' + from).addClass('bg-green');
                        $('#' + key + '_' + from).removeClass('bg-red');
                    }
                    $('#' + key + '_' + from).html('').html(current[key]);
                }
            }
                ...
                var decimal = 5;
                var amount = current[key]*$(".top-currency-dropdown a.selected").attr('value');
                if (amount >= 1) {
                    decimal = 2;
                }
                var formatted_amount = $(".top-currency-dropdown a.selected").attr('id') + "" + formatAmount(amount, decimal, false);
