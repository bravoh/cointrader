            ...
            <ul class="latest_postnav">
               ...
               <li style="margin-bottom:10px">
                  <div class="media">
                     ...
                     <div
                        class="media-body"><a href="{{ URL::to('/crypto-news') }}/{{$news->id}}/{{$news->alias}}" class="catg_title"> {{ str_limit(strip_tags($news->title), 65, '...') }} </a><br /><i class="fa fa-clock-o fa-fw" style="font-size:1em"></i>{{ date("Y-m-d", strtotime($news->publishedAt)) }} </div>
                  </div>
               </li>
               @endforeach 
            </ul>
            ...
            <ul class="spost_nav single-page-news">
               ...
               <li style="padding: 10px;">
                  <div class="media wow fadeInDown">
                     <a href="{{ URL::to('/crypto-news') }}/{{$news->id}}/{{$news->alias}}" class="media-left"> @if(file_exists('public/storage/' . $news->urlToImage)) <img alt="{{$news->title}}" title="{{$news->title}}" src="{{URL::asset('public/storage/' . $news->urlToImage)}}"> @else <img alt="{{$news->title}}" title="{{$news->title}}" src="{{$news->urlToImage}}"> @endif </a>
                     <div
                        class="media-body"><a href="{{ URL::to('/crypto-news') }}/{{$news->id}}/{{$news->alias}}" class="catg_title"> {{ str_limit(strip_tags($news->title), 65, '...') }} </a><br /><span class="ui label"><i class="fa fa-clock-o fa-fw" style="font-size:1em"></i>{{ date("d M Y", strtotime($news->publishedAt)) }}</span></div>
                  </div>
               </li>
               @endforeach
            </ul>
