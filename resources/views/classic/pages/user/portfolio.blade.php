...
<script src="{{ URL::asset('public/js/streaming-code.js') }}"></script>
<script>
                                ...
                                <table width="100%" class="table table-hover" id="dataTables-example">
                                    ...
                                    <tbody class="crypto-currencies-data">
                                        ...
                                        <tr class="odd gradeX" id="TABLE_ROW_{{$market->symbol}}">
                                            ...
                                            <td>
                                                @if(isset($market->coinDetails))
                                                    <a href="{{ makeUrl('currencies') }}/{{$market->alias}}">
                                                @endif
                                                {!! $market->image !!} {{$market->name}}
                                                @if(isset($market->coin_details))
                                                    </a>
                                                @endif    
                                            </td>
                                            <td class="price align-right" val="{{$market->price_usd}}" id="PRICE_{{$market->symbol}}" @foreach($market->coinRates as $rate) {{$rate['f_currency']}} = "{{$rate['price']}}" @endforeach></td>
