      ...
      <div class="row">
        <div class="col-lg-12">     
          <div class="loader"></div>                   
            ...
            <table width="100%" class="table dataTable no-footer dtr-inline collapsed table-hover" style="background: white;">
              <thead>
                  <tr>
              ...
              <tbody class="blockfolio-records">
                @if(count($coins) > 0)
                  @foreach ($coins as $coin)
                    <tr exchange="{{ $coin->exchange }}" pair="{{ str_replace('/', '-', $coin->pair) }}" quantity="{{ $coin->total_quantity }}" class="@if($coin->id % 2 == 0) odd @else even @endif gradeX">
