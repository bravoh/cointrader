      ...
      <div class="row">
        <div class="col-lg-12">     
          <div class="loader"></div>                   
            ...
            <table width="100%" class="table table-bordered table-hover" id="dataTables-example" style="background: white;">
              <thead>
                  <tr>
                ...
                @if(count($coins) > 0)
                  ...
                  @foreach ($coins as $coin)
                    @if($coin->total_quantity > 0)
                      <tr exchange="{{ $coin->exchange }}" pair="{{ str_replace('/', '-', $coin->pair) }}" quantity="{{ $coin->total_quantity }}" price="{{ $coin->price }}" class="@if($coin->id % 2 == 0) odd @else even @endif gradeX">
                          <td>{{ $coin->coin }}</td>
