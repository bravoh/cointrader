...
<style type="text/css">.block-explorer-tbl th{background-color: #2c3b41 !important;}.search-btn{width: 165px;}div.table-responsive {overflow-x: hidden !important;}</style>
@stop
@section('scripts')
        ...
        <div class="panel panel-default">
        	<div class="box box-success table-heading-class">
                <div class="box-header with-border">
                    <h1 class="box-title">@lang('blockexplorer.BLOCK_EXPLORER_HEADING')</h1>
                </div>
            </div>
            @include(getCurrentTemplate() . '.pages.block_explorer.block_search')
            <div class="panel-body">
            	<h3>@lang('blockexplorer.BLOCK') #: <span style="font-size: 18px;">{{ $block->height }}</span></h3>
            	<h3 style="word-break: break-all;">@lang('blockexplorer.BLOCK_HASH'): <span style="font-size: 18px;">{{ $block->hash }}</span></h3>
    			<div class="box box-success table-heading-class">
	                <div class="box-header with-border">
	                    <h1 class="box-title">@lang('blockexplorer.BLOCK_SUMMARY')</h1>
	                </div>
	            </div>
			    ...
			    <h3>@lang('blockexplorer.BLOCK_TXS')</h3>
			    <div class="table-responsive">
			        <table style="width: 100%;" class="table table-bordered dataTable no-footer dtr-inline collapsed" id="dataTables-example">
			            ...
			            <tbody>
			            	...
			            	<tr>
			            		<td>
			            			<a href="https://blockchain.info/tx/{{ $transactions->hash }}" rel="nofollow" target="_blank"> {{ str_limit($transactions->hash, 20) }} 
			            			</a>
			            		</td>
			            		<td class="align-right">{{ $transactions->tx_index }}</td>
			            		<td title="inputs / outputs" class="align-right">
			            			<span class="badge bg-green" style="width: 50px;">
