...
<div class="box box-success table-heading-class">
    <div class="box-header with-border">
        <h3 class="box-title">@lang('v7.ICOS_PAGE')</h3>
        <h1 class="sr-only">@lang('v7.ICOS_PAGE')</h1>
        <h2 class="sr-only">A complete list of crypto icos</h2>
        <h2 class="sr-only">Initial coin offerings</h2>
    </div>
    <p style="margin: 10px;">@lang('v7.ICOS_DESC')</p>
</div>
    ...
    <div class="row">
        ...
        <div class="col-4 col-xl-2 col-lg-2 col-md-3 col-sm-4 mb-3">    
            <a class="card" href="{{ makeUrl('crypto-ico') }}/{{ $ico->alias }}" title="{{ $ico->name }}">  
                ...
                <img class="img-fluid" alt="{{$ico->name}}" title="{{$ico->name}}" src="{{URL::asset('public/storage/' . $ico->image)}}" style="width: 100%;border-bottom: 1px solid #eee;height: 60px;" />
                @else
                <img class="img-fluid" alt="{{$ico->name}}" title="{{$ico->name}}" src="{{$ico->image}}" style="width: 100%;border-bottom: 1px solid #eee;height: 60px;" />
                @endif
                <div class="card-body">
