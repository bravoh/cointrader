...
@section('styles')
<style type="text/css">.table>tbody>tr>td, .table>tbody>tr>th, .table>tfoot>tr>td, .table>tfoot>tr>th, .table>thead>tr>td, .table>thead>tr>th{padding: 8px 15px !important;}</style>
@stop
@section('content')
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="box box-success table-heading-class">
                  <div class="box-header with-border">
                      <h3 class="box-title"> @lang('user.BLOCKFOLIO_TRANSACTIONS_PAGE_HEADING') </h3>
                              ...
                              @foreach ($coins as $coin)
                                <tr exchange="{{ $coin->exchange }}" pair="{{ str_replace('/', '-', $coin->pair) }}" quantity="{{ $coin->total_quantity }}">
                                    ...
                                    <td class="align-right">
                                      @if($coin->type == 1)
                                        <span class="badge bg-green" style="width: 80px;">@lang('user.HOLDING')</span>
                                      @elseif($coin->type == 2)  
                                        <span class="badge bg-red" style="width: 80px;">@lang('user.SOLD')</span>
                                      @else
                                        <span class="badge bg-yellow" style="width: 80px;">@lang('user.WATCH_ONLY')</span>
                                      @endif
                                    </td>
